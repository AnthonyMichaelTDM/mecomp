...
/// # Arguments
///
/// * `rpc_port` - the port that the subscribing application is listening on
///
/// # Errors
///
/// If the client cannot be initialized, an error is returned.
pub async fn init_application_publisher(
    rpc_port: u16,
) -> Result<ApplicationClient, std::io::Error> {
    let server_addr = (IpAddr::V4(Ipv4Addr::LOCALHOST), rpc_port);

    let mut transport = tarpc::serde_transport::tcp::connect(server_addr, Json::default);
    transport.config_mut().max_frame_length(usize::MAX);

    // ApplicationClient is generated by the service attribute. It has a constructor `new` that takes a
    // config and any Transport as input.
    Ok(ApplicationClient::new(client::Config::default(), transport.await?).spawn())
}
