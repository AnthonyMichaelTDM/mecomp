...
pub trait ComponentRender<Props> {
    ...
    fn render_border(&mut self, frame: &mut Frame<'_>, props: Props) -> Props;

    /// Render the view's content.
    fn render_content(&mut self, frame: &mut Frame<'_>, props: Props);

    /// Render the view (border and content).
    fn render(&mut self, frame: &mut Frame<'_>, props: Props) {
